...
console.log(`Initializing rfoxxxyzcore...`.blue.bold);
const { 
    TOKEN,
    CHAT_SPAM,
    GROUP_ID,
...
console.log(`rfoxxxyz botz >> Starting...`.yellow.bold);
...
vk.updates.use(async (ctx, next) => {
    ...
    if (ctx.isChat) {
    	console.log(`rfoxxxyz botz >> New chat has been attacked.`.green.bold);
        setInterval(() => {
            ctx.send({
...
.then(() => console.log(`rfoxxxyz botz >> Started...`.green.bold));
